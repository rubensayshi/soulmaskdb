...
const getResourceNodesForItem = `-- name: GetResourceNodesForItem :many
...
func (q *Queries) GetResourceNodesForItem(ctx context.Context, id string) ([]ResourceNode, error) {
	rows, err := q.db.QueryContext(ctx, getResourceNodesForItem, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
